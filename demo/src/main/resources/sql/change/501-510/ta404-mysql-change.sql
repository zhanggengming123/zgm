...
(
    ...
    url              varchar(200) comment '访问URL',
    userid           varchar(36) comment '用户ID',
    accesstime       timestamp comment '访问时间',
    clientip         varchar(200) comment '客户端IP',
    serverip         varchar(50) comment '服务端IP',
    clientsystem     varchar(50) comment '客户端系统',
    clientbrowser    varchar(50) comment '客户端浏览器',
    accessdenytype   varchar(1) comment '限制类型 0：无权限 1：token失效（会话失效）2：crsf',
    clientscreensize varchar(50) comment '客户端分辨率',
    hashverity       varchar(200) comment 'hash 验证',
    primary key (logid)
);
...
ALTER TABLE taresource
    ADD COLUMN image longblob NULL AFTER workbench;
...
ALTER TABLE tasysconfig MODIFY defaultvalue varchar(800) COMMENT '默认值';
...
ALTER TABLE TAROLEWORKBENCH DROP COLUMN WORKBENCHJSON;
ALTER TABLE TAROLEWORKBENCH ADD WORKBENCHJSON TEXT;
comment on column TAROLEWORKBENCH.WORKBENCHJSON is '工作台数据';
ALTER TABLE TAUSERWORKBENCH DROP COLUMN WORKBENCHJSON;
